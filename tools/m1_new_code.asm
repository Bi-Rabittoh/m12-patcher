...
lowerencounterrate:
  ...
  bne  +
  ldr  r1,=#enratetable  // SCRIVERE "enratetable2" PER AZZERARE DEL TUTTO GLI INCONTRI CASUALI DURANTE LA DEBUG MODE
...
//----------------------------------------------------------------------------------------
// Hack which changes the dollar sign's position in shops so it's after the price [merito di Lorenzooone]
...
parsecopy:
  ...
  + 
  cmp  r3,#0xF2; bne +; bl control_code_F2; b .loop_start
  + 
...
//----------------------------------------------------------------------------------------
// this adds "Il gruppo di " if there's more than one character in the party
...
//----------------------------------------------------------------------------------------
// CODICE DI CONTROLLO PER FAR APPARIRE L'ARTICOLO DAVANTI ALL'OGGETTO
// QUANDO VIENE USATO FUORI DA UNA BATTAGLIA

control_code_F2:
  push {r0,r2,lr}

  ldr  r0,=#0x30007D4       // this has the current item # (hopefully all the time)
  ldrb r0,[r0,#0x0]         // load item #
  lsl  r0,r0,#0x4
  ldr  r2,=#0x8FFE000
...
add_space_to_enemy_name:
  ...
  mov  r4,#0x01
  strb r4,[r5,#0x0]
  sub  r0,#0x6C      // MODIFICA LA LETTERA IDENTIFICATIVA DEL NEMICO PER UTILIZZARE LA CORRISPONDENTE VARIANTE IN GRASSETTO
  strb r0,[r5,#0x1]
  add  r5,#0x2
  pop  {pc}
